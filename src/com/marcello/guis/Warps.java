...
public class Warps implements Listener, CommandExecutor
{
    ...
    public void Clicar(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && p.getItemInHand().getType() == Material.BOOK && p.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase("§bWarps §7(Clique)")) {
            p.chat("/warps");
        }
    }
