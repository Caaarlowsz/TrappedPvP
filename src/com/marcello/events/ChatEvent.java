...
public class ChatEvent implements Listener
{
    ...
    public void modificarchat(final AsyncPlayerChatEvent evento) {
        ...
        if (jogador.hasPermission("chat.cor")) {
            evento.setFormat(String.valueOf(jogador.getDisplayName()) + " §7(" + Elos.getIconElo(jogador) + "§7)§f: " + evento.getMessage().replace("%", " Porcento(s)"));
            evento.setFormat(evento.getFormat().replace("&", "§"));
        }
        else {
            evento.setFormat(String.valueOf(jogador.getDisplayName()) + " §7(" + Elos.getIconElo(jogador) + "§7)§f: " + evento.getMessage());
        }
