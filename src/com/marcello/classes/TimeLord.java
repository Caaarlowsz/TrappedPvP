...
public class TimeLord implements Listener
{
    ...
    public void timelordkit(final PlayerInteractEvent event) {
        ...
        if ((event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) && player.getItemInHand().getType() == Material.WATCH && KitAPI.getKit(player) == "TimeLord") {
    ...
    public void onPlayerMove(final PlayerMoveEvent event) {
        ...
        if (TimeLord.freeze.contains(player.getName()) && !TimeLord.freezing.contains(player.getName())) {
            ...
            player.sendMessage("§c§lKIT §7um player com o kit §4§lTIMELORD §7paralizou voc\u00ea aguarde alguns segundos");
