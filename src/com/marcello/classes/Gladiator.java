...
public class Gladiator implements Listener
{
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && KitAPI.getKit(p) == "Gladiator") {
                ...
                if (this.generateGLASS) {
                    ...
                    for (int bX = -10; bX <= 10; ++bX) {
                        for (int bZ = -10; bZ <= 10; ++bZ) {
                            for (int bY = -1; bY <= 10; ++bY) {
                                ...
                                if (!b.isEmpty()) {
                                    event.setCancelled(true);
                                    p.sendMessage("§c§lGLADIATOR §7Possuem blocos acima n\u00e3o \u00e9 possivel criar arena aqui");
                                    return;
                                }
                                if (bY == 10) {
                    ...
                    p.sendMessage("§c§lGLADIATOR §7Voc\u00ea desafiou um player para uma batalha, ambos possuem 5 segundos de invesibilidade para se preparar");
                    p.sendMessage("§c§lGLADIATOR §7Observa\u00e7\u00e3o, se passar 4 minutos na arena e ninguem morrer ambos voltaram para os locais de inicio, \u00e9 a arena ser\u00e1 deletada");
                    r.sendMessage("§c§lGLADIATOR §7Voc\u00ea foi desafiado por um player para uma batalha, ambos possuem 5 segundos de invesibilidade para se preparar");
                    r.sendMessage("§c§lGLADIATOR §7Observa\u00e7\u00e3o, se passar 4 minutos na arena e ninguem morrer ambos voltaram para os locais de inicio, \u00e9 a arena ser\u00e1 deletada");
                    ...
                    this.id2 = Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.getInstace(), (Runnable)new Runnable() {
                        @Override
                        public void run() {
                    ...
                    this.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.getInstace(), (Runnable)new Runnable() {
                        @Override
                        public void run() {
                            if (Gladiator.lutando.containsKey(p.getName()) && Gladiator.lutando.get(p.getName()).equalsIgnoreCase(r.getName()) && Gladiator.lutando.containsKey(r.getName()) && Gladiator.lutando.get(r.getName()).equalsIgnoreCase(p.getName())) {
                                ...
                                p.sendMessage("§c§lGLADIATOR §7Ninguem venceu a batalha, voc\u00ea voltou para o local de inico");
                                r.sendMessage("§c§lGLADIATOR §7Ninguem venceu a batalha, voc\u00ea voltou para o local de inico");
    ...
    public void onDeathGladiator(final PlayerDeathEvent e) {
        ...
        if (Gladiator.lutando.containsKey(p.getName())) {
            ...
            k.teleport(old);
            k.sendMessage("§c§lGLADIATOR §7Voc\u00ea venceu a batalha contra §c" + p.getName() + ChatColor.GREEN + "§a!");
            Bukkit.getScheduler().cancelTask(this.id1);
            Bukkit.getScheduler().cancelTask(this.id2);
