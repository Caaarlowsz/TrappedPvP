                    ...
                    public void run() {
                        if (Ninja.NinjaTime.get(p.getName()) <= 0) {
                            if (KitAPI.getKit(p) == "Ninja" && KitAPI.KitDelay.containsKey(p.getName())) {
                                p.sendMessage("§c§lKIT §7Voc\u00ea ficou muito tempo sem hitar §c" + Ninja.NinjaPlayer.get(p.getName()) + " §7n\u00e3o pode mais se teleportar at\u00e9 ele");
                            }
    ...
    public void Agachar(final PlayerToggleSneakEvent e) {
        ...
        if (p.isSneaking() && KitAPI.getKit(p) == "Ninja") {
            ...
            if (!Ninja.NinjaPlayer.containsKey(p.getName())) {
                p.sendMessage("§c§lKIT §7Voc\u00ea n\u00e3o hitou ningu\u00e9m, hite alguem para pode utilizar seu kit §4§lNINJA");
                return;
            }
            ...
            p.teleport((Entity)t);
            p.sendMessage("§c§lKIT §7Voc\u00ea foi teleportado at\u00e9 §c" + Ninja.NinjaPlayer.get(String.valueOf(p.getName()) + "§7 usando seu kit §4§lNINJA"));
