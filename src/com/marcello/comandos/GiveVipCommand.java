		...
		if (cmd.getName().equalsIgnoreCase("givevip")) {
			...
			if (args.length == 2) {
				...
				if (vipgroup.equalsIgnoreCase("Ultimate")) {
					p.sendMessage("2§LGIVEVIT §7O vip §d§lULTIMATE §7foi dado ao jogador §a" + jogador.getName() + "§e("
							+ jogador.getUniqueId() + "§e)");
					Bukkit.dispatchCommand((CommandSender) Bukkit.getConsoleSender(),
							"pex user " + jogador.getName() + " group set ultimate");
					return true;
				}
				if (vipgroup.equalsIgnoreCase("Premium")) {
					p.sendMessage("2§LGIVEVIT §7O vip §6§lPREMIUM §7foi dado ao jogador §a" + jogador.getName() + "§e("
							+ jogador.getUniqueId() + "§e)");
					Bukkit.dispatchCommand((CommandSender) Bukkit.getConsoleSender(),
							"pex user " + jogador.getName() + " group set premium");
					return true;
				}
				if (vipgroup.equalsIgnoreCase("light")) {
					p.sendMessage("2§LGIVEVIT §7O vip §a§lLIGHT §7foi dado ao jogador §a" + jogador.getName() + "§e("
							+ jogador.getUniqueId() + "§e)");
					Bukkit.dispatchCommand((CommandSender) Bukkit.getConsoleSender(),
							"pex user " + jogador.getName() + " group set light");
					return true;
				}
			}
		}
