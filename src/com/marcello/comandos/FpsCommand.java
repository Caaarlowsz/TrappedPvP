...
public class FpsCommand extends WarpsAPI implements CommandExecutor {
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (args.length == 0) {
			...
			if (KitAPI.getKit(p) == "Nenhum") {
				FpsCommand.segundos.add(p.getDisplayName());
				Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin) Main.getInstace(), (Runnable) new Runnable() {
					@Override
					public void run() {
						if (FpsCommand.segundos.contains(p.getDisplayName())) {
							p.sendMessage("§2§LWARP §7Voc\u00ea entrou para a warp §a§lFPS");
							p.playSound(p.getLocation(), Sound.NOTE_PLING, 1.0f, 1.0f);
							WarpsAPI.ir(p, "Fps");
							p.setLevel(0);
							FpsCommand.segundos.remove(p.getDisplayName());
							KitAPI.Fps(p);
			...
			WarpsAPI.Set(p, "Fps");
			p.sendMessage("§2§lWARP §7Voc\u00ea setou a arena §a§lFPS");
		}
		return false;
	}
}
