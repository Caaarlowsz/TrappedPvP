		...
		if (cmd.getName().equalsIgnoreCase("setmotd") && sender instanceof Player) {
			...
			if (args.length == 1) {
				if (args[0].equalsIgnoreCase("reset")) {
					...
					player.sendMessage("§b§lSETMOTD §7A atual MOTD setada: "
							+ MOTDCommand.changeMOTD.toString().replace("[", "").replace("]", ""));
					return true;
				} else {
					if (!MOTDCommand.changeMOTD.isEmpty()) {
						player.sendMessage(
								"§b§lSETMOTD §7A Motd j\u00e1 est\u00e1 alterada, Resete-a e Tente novamente.");
						return true;
					}
